    ...
    <section className="panel">
      ...
      <div className="threat-list">
        {sortedClusters.slice(0, 5).map((cluster, index) => {
          const threatPercent = Math.round((cluster.threat_score ?? 0) * 100);
            ...
            >
              ...
              <div className="cluster-score">
                <div className="progress-track">
                  <div
                    className={`progress-fill ${getThreatProgressTone(cluster.threat_level ?? "low")}`}
                    style={{ width: `${Math.max(6, Math.min(100, threatPercent))}%` }}
                  />
                </div>
              </div>
            </div>
...
function getThreatProgressTone(level: ThreatLevel) {
